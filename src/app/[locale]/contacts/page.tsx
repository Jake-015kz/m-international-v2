      ...
      <section className="py-8 md:py-16">
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12">
            ...
            <ScrollReveal>
              <div>
                <h2 className="font-unbounded font-bold text-lg md:text-xl text-text-primary mb-4 md:mb-6">{t("title")}</h2>
                <div className="space-y-3 md:space-y-5">
                  {[
                    { icon: <MapPin className="w-5 h-5" />, title: t("address"), lines: [t("addressText")] },
                    { icon: <Phone className="w-5 h-5" />, title: t("phone"), lines: ["+7 (777) 123-45-67", "+7 (727) 987-65-43"] },
                    { icon: <Mail className="w-5 h-5" />, title: t("email"), lines: ["info@m-international.kz", "support@m-international.kz"] },
                    { icon: <Clock className="w-5 h-5" />, title: t("officeHours"), lines: [t("officeHoursText"), t("note")] },
                  ].map((item) => (
                ...
                <div className="mt-4 md:mt-6 p-3 md:p-4 rounded-2xl bg-surface-elevated border border-border-subtle">
                  <div className="flex items-center gap-2 mb-1.5 md:mb-2">
                    ...
                    <span className="font-onest font-bold text-xs md:text-sm text-text-primary">{t("description")}</span>
                  </div>
                  <p className="text-[11px] md:text-xs text-text-secondary font-onest">{t("description")}</p>
                </div>
              </div>
            </ScrollReveal>
            ...
            <ScrollReveal delay={0.1}>
              <div className="p-4 md:p-8 rounded-2xl bg-surface-elevated border border-border-subtle">
                <h2 className="font-unbounded font-bold text-lg md:text-xl text-text-primary mb-1.5 md:mb-2">{t("title")}</h2>
                <p className="text-xs md:text-sm text-text-secondary font-onest mb-4 md:mb-6">{t("description")}</p>
                <form className="space-y-3 md:space-y-4" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <div>
                    <label className="block text-[11px] md:text-xs font-onest font-medium text-text-secondary mb-1 md:mb-1.5">{t("email")}</label>
                  ...
                  <div>
                    <label className="block text-[11px] md:text-xs font-onest font-medium text-text-secondary mb-1 md:mb-1.5">{t("phone")}</label>
          ...
          <ScrollReveal>
            <div className="rounded-2xl overflow-hidden border border-border-subtle h-48 md:h-80 bg-surface-elevated flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="font-onest text-xs md:text-sm text-text-secondary">{t("addressText")}</p>
                <p className="text-[11px] md:text-xs text-text-tertiary font-onest mt-1">{t("note")}</p>
              </div>
            </div>
          </ScrollReveal>
        </Container>
