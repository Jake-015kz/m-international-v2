    ...
    <section className="relative min-h-[100dvh] bg-[#FAFAFA] overflow-hidden">
      ...
      >
        <div className="container mx-auto px-4">
          <div className="flex justify-center items-center gap-8 md:gap-12 text-zinc-400 font-onest text-sm">
            {stats.map((stat, i) => (
              <div key={i} className="flex items-center gap-8 md:gap-12">
                {i > 0 && <div className="w-px h-6 bg-zinc-200" />}
                <span>
                  <strong className="font-unbounded text-base md:text-lg font-bold text-[#1A1A1A]">{stat.value}</strong>
                  {" "}{stat.label}
                </span>
              </div>
            ))}
      ...
      <motion.div
        ...
        className="lg:hidden flex justify-center items-center gap-6 sm:gap-8 mt-10 sm:mt-12 text-zinc-400 font-onest text-xs sm:text-sm"
      >
        {stats.map((stat, i) => (
          <div key={i} className="flex items-center gap-6 sm:gap-8">
            {i > 0 && <div className="w-px h-6 sm:h-8 bg-zinc-200" />}
            <span>
              <strong className="font-unbounded text-sm sm:text-base font-bold text-[#1A1A1A]">{stat.value}</strong>
              {" "}{stat.label}
            </span>
          </div>
        ))}
